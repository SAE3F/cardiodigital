...
import { FARMACOS, getInteractionsForSelection, Farmaco } from '@/lib/data/interacciones'
import { Search, Pill, AlertTriangle, X, ShieldAlert, CheckCircle2 } from 'lucide-react'
...
export default function InteraccionesPage() {
  ...
  // Filtrar fármacos por búsqueda (excluyendo los ya seleccionados)
  ...
  const selectedFarmacos = selectedIds.map(id => FARMACOS.find(f => f.id === id)!)
  const interacciones = getInteractionsForSelection(selectedIds)

  const toggleFarmaco = (id: string) => {
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto pb-24">
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm">
          Evaluación cruzada offline de interacciones cardiovasculares (Basado en ACC/AHA, ESC y Lexicomp).
        </p>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          placeholder="Buscar fármaco (ej. Amiodarona, Sildenafil)..."
          value={searchQuery}
        ...
        {searchQuery && (
          <div className="absolute z-10 w-full mt-2 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
            {filteredFarmacos.length === 0 ? (
              <div className="p-4 text-slate-400 text-sm text-center">No se encontraron fármacos</div>
            ) : (
              <ul className="max-h-60 overflow-auto">
                {filteredFarmacos.map(f => (
                  <li 
                    key={f.id}
                    onClick={() => toggleFarmaco(f.id)}
      ...
      {selectedFarmacos.length > 0 && (
        <div className="mb-8">
          <h3 className="text-sm font-medium text-slate-400 mb-3">Receta actual ({selectedFarmacos.length}):</h3>
          <div className="flex flex-wrap gap-2">
            {selectedFarmacos.map(f => (
              <Badge 
                key={f.id}
                variant="secondary"
                className="pl-3 pr-2 py-1.5 text-sm bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-200 gap-2 flex items-center"
              >
        ...
        {selectedFarmacos.length < 2 ? (
          <div className="text-center py-16 px-4 bg-slate-900/50 rounded-2xl border border-slate-800 border-dashed">
            ...
            <p className="text-slate-500 mt-2 text-sm max-w-sm mx-auto">
              Seleccioná 2 o más fármacos en el buscador para analizar posibles interacciones cardiovasculares.
            </p>
          </div>
        ) : interacciones.length === 0 ? (
          <div className="text-center py-12 px-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20">
            <CheckCircle2 className="w-12 h-12 text-emerald-500 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-emerald-400">Sin interacciones críticas detectadas</h3>
            <p className="text-emerald-500/70 mt-2 text-sm max-w-sm mx-auto">
              No se encontraron interacciones mayores o contraindicaciones entre los fármacos seleccionados en nuestra base de datos.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-slate-200 flex items-center gap-2">
              ...
              Se detectaron {interacciones.length} interaccione(s)
            </h3>
